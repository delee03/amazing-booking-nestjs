...
export class AuthService {
  ...
  async validateUser(email: string, password: string): Promise<User> {
    try {
      ...
      console.log('User found in validateUser:', user ? user.email : 'Validate: Not found User');
  ...
  async signIn(signInDto: SignInDto): Promise<{ statusCode: number; content: any; token: string; dateTime: string }> {
    try {
      ...
      console.log('Payload: Mail(' + payload.email + ') With id:' + payload.sub);
